...
### ⚠️ 字段类型 → 样式 强绑定规则（LOCKED）

每种字段类型只能使用下方指定的一种样式，不得混用。如需新增字段类型，须先在此表登记再实现。

| 字段类型 | 使用样式 | 关键 CSS |
|---|---|---|
| **text**（单行文本） | Text style | `.ff-row` + `.ff-value__text` |
| **longtext**（长文本） | Long text style | `.ff-row.ff-row--longtext` + `.ff-textarea` |
| **boolean**（是/否） | Boolean style | `.ff-row` + `.ff-radio-group > .ff-radio` |
| **picklist**（单选） | Picklist style | `.ff-row` + `.ff-trigger`（chevron）或 `.ff-segment` |
| **date**（日期） | Date style | `.ff-row` + `.ff-trigger` + 日历图标（`线性-日历.svg`） |
| **datetime**（日期时间） | Datetime style | `.ff-row` + `.ff-trigger` + 时钟图标（`线性-时间.svg`） |
| **lookup**（关联查找） | Lookup style | `.ff-row` + `.ff-trigger`（暂参考 Date style，待独立定义） |

**Text style 仅用于 `text`（单行文本）字段**，不得用于 date / datetime / lookup。
...
│ Identity card — 双版本切换                    │
│  ┌─ 默认：简洁版 (~68) ─────────────────────┐│
│  │  白底 · 44px avatar · 姓名 · 展开按钮    ││ ← local .compact-identity
│  └───────────────────────────────────────────┘│
│  ┌─ 展开后：完整版 (~160) ──────────────────┐│
│  │  橙色渐变 · 60px avatar · 关注数 · 关注  ││ ← detail-identity.css · .ea-detail-identity
│  │  pill · 认证 chips · meta line · 收起钮  ││
│  └───────────────────────────────────────────┘│
│  点击展开按钮（线性-全屏.svg）切换至完整版；   │
│  完整版右上角收起按钮（线性-退出全屏.svg）     │
│  回到简洁版。JS classList.toggle 驱动。        │
├──────────────────────────────────────────────┤
│ Tab strip                                    │ ← list-shell.css · .ea-list-tabs
│   基本信息 / 我的战友 / … + overflow 按钮    │
│   overflow 图标：线性-更多2.svg              │
├──────────────────────────────────────────────┤
│ HCP Insights — 医生要览 (AI 洞察卡)           │ ← hcp-insights.css
│   .ea-hcp-insights-frame（蓝渐变外框）        │   仅在「基本信息」tab 内显示
│   └── .ea-hcp-insights（白底卡片）            │
│       医生动向 · 医生观念 · 收起全文          │
│       └── .ea-hcp-insights__plan-cta          │
│           （帮我规划下次拜访，青→蓝渐变）      │
│       └── .ea-hcp-insights__ai-mark           │
├──────────────────────────────────────────────┤
│ ┌── 分组 1 ────────────────────────────────┐ │
...
| 身份卡（简洁版，默认） | local CSS in page | `.compact-identity`（白底 · 44px avatar · 展开按钮） |
| 身份卡（完整版，展开后） | `detail-identity.css` | `.ea-detail-identity`（橙色渐变 · 60px avatar · 关注/认证/meta） |
| Tab 切换 | `list-shell.css` | `.ea-list-tabs`，overflow 图标用 `线性-更多2.svg` |
| AI 洞察卡（基本信息 tab） | `hcp-insights.css` | `.ea-hcp-insights-frame > .ea-hcp-insights` |
...
- ❌ Don't skip the compact identity header and go straight to `ea-detail-identity` — the compact version is the default entry state; the full version is revealed on demand via the expand button
...
| 2a. 身份卡·简洁版（默认） | local page CSS | `.compact-identity`（白底 44px，展开按钮） |
| 2b. 身份卡·完整版（展开后） | ⭐ `detail-identity.css` | `.ea-detail-identity`（橙色渐变 60px） |
| 3. Tab 条 | 🔁 `list-shell.css` | `.ea-list-tabs`，overflow = `线性-更多2.svg` |
| 4. HCP Insights（基本信息 tab） | ⭐ `hcp-insights.css` | `.ea-hcp-insights-frame` > `.ea-hcp-insights` |
| 5. 分组 section header | ⭐ `form-section.css` | `.ea-form-section` |
| 6. Form 字段行（view 模式） | 🔁 `forms.css` | `.ff-row` / `.ff-label` / `.ff-value` |
| 7. 底部 BAB（2 按钮） | 🔁 `actions.css` | `.ea-bab` |
...
40-tall strip used between groups of form rows on detail (and edit) pages. White outer, with a 32-tall inner pill in soft blue wash (`--color-bg-extra-blue-2`) hosting a 14/500 title in **`--color-section-blue` (#2069D6)** (`基本信息` / `联系方式` / etc.). Optional `.ea-form-section__count` slot on the right for "(共 7 项)" or similar. Preview: `preview/component-form-section.html`.
...
### ⚠️ Rule: list row avatar icons must come from `assets/icons/Homepage/`

Every `.ea-avatar` inside a list row uses a **Homepage business icon**, not a 线性 or 面性 icon. The correct markup is always `ea-avatar--homepage` + `<img>`, never `ea-avatar--soft` + `ea-avatar__glyph` (mask pattern).

**Why Homepage icons:**
The Homepage business icon is the same graphic the user tapped on the workbench to enter the list. Reusing it in the list avatar creates a visual handoff — the user immediately recognises "I tapped 我的医生, these rows are doctors." Swapping in a 线性 icon that looks vaguely similar breaks this affordance.

**Correct pattern:**
```html
<div class="ea-avatar ea-avatar--homepage">
  <img src="../assets/icons/Homepage/Homepage-我的医生.svg" alt="">
</div>
```

**Wrong patterns — do not use:**
```html
<!-- ✗ mask pattern collapses multicolor SVG to a single tint -->
<div class="ea-avatar ea-avatar--soft">
  <span class="ea-avatar__glyph" style="--icon: url(线性-用户.svg)"></span>
</div>

<!-- ✗ inline hand-drawn SVG — not from icon library at all -->
<div style="background: #DBE6F4 url('data:image/svg+xml,...') center/24px no-repeat; ..."></div>
```

**Icon-to-list mapping** (must match the Homepage entry that navigated to this list):

| List type | Avatar icon |
|---|---|
| 我的医生 | `Homepage-我的医生.svg` |
| 我的医院 | `Homepage-我的医院.svg` |
| 我的产品 | `Homepage-我的产品.svg` |
| 目标医生 | `Homepage-目标医生.svg` |
| 外部联系人 | `Homepage-外部联系人.svg` |
| 通用（无固定实体类型） | `Homepage-通用.svg` |
| Other entities | Pick the matching `Homepage-*.svg` — never substitute a 线性/面性 icon |

For demo/preview pages where the entity type is not fixed, use `Homepage-我的医生.svg` as the default placeholder (it is the most common CRM entity in this system).
...
- **Using double quotes inside a double-quoted `style` attribute.** `style="--icon: url("data:...")"` breaks the HTML attribute at the first inner `"` — the browser parses `style="--icon: url("` and discards everything after. The result is `--icon` is never set → mask fails → colored square. **Always use single quotes inside `url()` when the data URL is inside a `style="..."` attribute:** `style="--icon: url('data:...')"`. This is the only correct form for inline data URLs in HTML attributes.
- **Leaving `<` and `=` unencoded in the SVG data URL.** When calling `urllib.parse.quote()`, using a permissive `safe=` parameter that includes `<`, `>`, `/`, `=`, `:` leaves those characters raw inside the `url()` value. CSS parsers treat bare `<` as invalid inside a `url()` token and silently discard the value — same result as if `--icon` was never set. **Always use `safe=''` (or at most `safe='-._~'`) when encoding SVG data URLs for CSS.** The correct output starts with `url('data:image/svg+xml;charset=utf-8,%3Csvg%20...` where `<` is `%3C` and `=` is `%3D`.

### ⚠️ Rule: icon data URLs must be copied from reference pages, not re-encoded manually

This rule applies to **every file that uses this design system** — component preview pages, demo screens, prototype builds, storybook fixtures, handoff specs, production Vue/React components, anything. There are no exceptions based on file type or purpose.

**Root cause of the problem.** When any DS-based file is assembled, it's tempting to grab the SVG source from `assets/icons/` and encode it on the spot — via Python, a data-URL tool, or by hand. Even when the encoding logic is correct, the wrong SVG can be picked. The result is an icon that is visually plausible (same size, same color) but semantically wrong — e.g., the `ea-list-tabs__overflow` button showing a bullet-list glyph instead of the required 三横线 hamburger.

This error is **silent**: no console warning, no layout break, just the wrong shape. It will survive code review unless the reviewer already knows the exact expected glyph.

**The rule.** For any DS component's icon that already appears in a reference component preview page, copy its `--icon` data URL **verbatim** from that reference page. Never re-encode the SVG manually when a canonical URL already exists.

Reference pages to use as source of truth:

| Component / icon family | Reference file |
|---|---|
| List shell — tabs overflow (☰), month nav arrows | `preview/list-shell.html` |
| Filter bar — filter icon, sort icon, dropdown chevron | `preview/filters.html` |
| Search pill — search glyph | `preview/search.html` (or copy from any list demo that already uses it) |
| Bottom action bar — add FAB | `preview/bottom-actions.html` |
| Navbar — back arrow, more (⋯) | `preview/navbar.html` |
| Form fields — trigger arrow, clear × | `preview/form-fields.html` |
| Detail page — section icons, follow star | `preview/detail.html` |
| All 线性 / 面性 glyphs | `preview/icons.html` (click-to-copy data URL) |

**Workflow for building any DS-based file:**

1. Identify every icon the file needs.
2. Find each one in the table above and open the reference file.
3. `Cmd+U` (View Source) or DevTools → search for the component class (e.g. `ea-list-tabs__overflow-icon`) to locate the exact inline `style="--icon: url(...)"` value.
4. Copy that entire `url(data:image/svg+xml;...)` string verbatim and paste it into the new file.
5. **Do not** extract the SVG file path from `assets/icons/`, run it through `urllib.parse.quote`, and hope the path ends up pointing to the right glyph — this is how the wrong icon gets in.

**The only time manual encoding is needed** is when a genuinely new icon has never been used in any existing reference page. In that case:

1. Use the `stripClipRefs` + `urllib.parse.quote` Python pipeline (documented under *Icon encoding pipeline* below).
2. Add the result to the appropriate **reference preview page** first.
3. Then copy it verbatim into the consuming file.

This keeps all canonical icon URLs in one authoritative place and makes wrong-icon bugs detectable by diffing against the reference page.
